...
public class ValidateCC {
	...
	public static int sumOfDoubleEvenPlace(String number) {
		...
		// Start from the second digit from right and move two positions to the left each time around the loop
		for (int i = number.length() - 2; i >= 0; i-=2) {
			int n = Integer.parseInt("" + number.charAt(i));
			result += getSumDigits(2 * n); // If doubling of a digit results in a two-digit number, add up the two digits to get a single-digit number
		}
	...
	public static int sumOfOddPlace(String number) {
		...
		// Start from the first digit from right and move two positions to the left each time around the loop
