...
public class LottoTickets {
	public static void main(String[] args) {
		...
		boolean[] isIncluded = new boolean[99]; // Default element values are false
		...
		do {
			...
			// Update the array isIncluded
			for (int i = 0; i < ticket.length; i++)
				isIncluded[ticket[i] - 1] = true;

			// Check if each number between 1 and 99 inclusive has been included
			// at least once in the tickets generated so far
			boolean allIncluded = true; // Assume all numbers are included
			
			for (int i = 0; i < isIncluded.length; i++)
				if (!isIncluded[i]) { // Found a number that is not included
					allIncluded = false; // Set allIncluded to false
